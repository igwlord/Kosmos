...
export function Features() {
  ...
  return (
    <section className="w-full max-w-7xl px-panel-margin py-32">
      <div
        ref={headingRef}
        className={`flex flex-col items-center mb-20 transition-all duration-700 ease-out ${headingVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-module-gap auto-rows-[450px]">
        {/* Main UI Block (Voice) */}
        <div
          ref={voiceRef}
          className={`md:col-span-8 glass-panel rounded-2xl p-8 flex flex-col relative overflow-hidden group transition-all duration-700 ease-out ${voiceVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          id="main-ui"
        >
        ...
        <div
          ref={fxRef}
          className={`md:col-span-4 glass-panel rounded-2xl p-8 flex flex-col relative overflow-hidden group transition-all duration-700 ease-out delay-100 ${fxVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          id="effects"
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="mt-auto rounded-xl overflow-hidden border border-white/5 shadow-2xl relative h-48">
        ...
        <div
          ref={modRef}
          className={`md:col-span-12 glass-panel rounded-2xl p-10 flex flex-col md:flex-row gap-12 relative overflow-hidden items-center group transition-all duration-700 ease-out delay-200 ${modVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          id="modulation"
        >
